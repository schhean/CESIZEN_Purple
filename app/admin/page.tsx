...
export default function AdminDashboardPage() {
  ...
  const toggleUserStatus = async (id: number, currentStatus: boolean) => {
    ...
    setUsers(
      users.map((u) =>
        u.id_utilisateur === id ? { ...u, actif: newStatus } : u,
      ),
    );
    try {
  ...
  const toggleArticleStatus = async (id: number, currentStatus: boolean) => {
    ...
    setArticles(
      articles.map((a) =>
        a.id_article === id ? { ...a, est_publie: newStatus } : a,
      ),
    );
    try {
  ...
  const handleUpdateArticle = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/articles/${editingArticle.id_article}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
      ...
      if (response.ok) {
        ...
        setArticles(
          articles.map((a) =>
            a.id_article === updated.id_article ? updated : a,
          ),
        );
  ...
  const handleUpdateExercice = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/exercices/${editingExercice.id_exercice}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
      ...
      alert("Erreur modification exercice");
    }
  };
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 relative">
            ...
            {isUsersLoading ? (
              ...
              users.map((user) => (
                <div
                  ...
                  className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-gray-200 dark:border-zinc-800 shadow-sm flex flex-col gap-3"
                >
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-gray-900 dark:text-white text-base">
                      #{user.id_utilisateur} - {user.prenom} {user.nom}
                    </span>
                    <span
                      className={`px-2 py-1 text-[10px] font-bold rounded-md ${user.role === "ADMIN" ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}
                    >
                      {user.role}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center pt-3 border-t border-gray-100 dark:border-zinc-800">
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-1 text-[10px] font-semibold rounded-md ${user.actif ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                    >
                      {user.actif ? (
                    ...
                    <div className="flex gap-2">
                      <button
                        className={`p-2 rounded-lg bg-gray-50 dark:bg-zinc-800 ${user.actif ? "text-amber-600" : "text-emerald-600"}`}
                        onClick={() =>
                          toggleUserStatus(user.id_utilisateur, user.actif)
                        }
                      >
                        {user.actif ? (
                      ...
                      <button
                        className="p-2 text-red-600 bg-red-50 dark:bg-red-900/10 rounded-lg"
                        onClick={() =>
                          deleteUser(user.id_utilisateur, user.nom)
                        }
                      >
                ...
                {users.map((user) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-sm">
                      <span
                        className={`px-2.5 py-1 text-xs font-semibold rounded-full ${user.role === "ADMIN" ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}
                      >
                        {user.role}
                      </span>
                    </td>
                    <td className="p-4 text-sm">
                      <span
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full ${user.actif ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                      >
                        {user.actif ? (
                    ...
                    <td className="p-4 flex items-center justify-end gap-2">
                      <button
                        className={`p-2 rounded-lg ${user.actif ? "text-amber-600" : "text-emerald-600"}`}
                        onClick={() =>
                          toggleUserStatus(user.id_utilisateur, user.actif)
                        }
                      >
      ...
      {activeTab === "articles" && (
        <div className="w-full">
          ...
          {showAddArticleForm && (
            <form
              className="bg-white dark:bg-zinc-900 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-800 mb-8"
            ...
            >
              <div className="grid gap-4 mb-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewArticle({ ...newArticle, resume: e.target.value })
                    }
                  />
                </div>
                <div>
                ...
                <label className="flex items-center gap-2 mt-2 cursor-pointer">
                  <input
                    checked={newArticle.est_publie}
                    className="w-4 h-4"
                    type="checkbox"
                    onChange={(e) =>
                      setNewArticle({
                        ...newArticle,
                        est_publie: e.target.checked,
                      })
                    }
                  />
              ...
              <button
                className="w-full sm:w-auto px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg"
                type="submit"
              >
                Créer l'article
              </button>
            ...
            {articles.map((article) => (
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  <span className="font-bold text-gray-900 dark:text-white text-base line-clamp-2">
                    {article.titre}
                  </span>
                  <span
                    className={`px-2 py-1 text-[10px] font-bold rounded-md whitespace-nowrap ${article.est_publie ? "bg-emerald-100 text-emerald-700" : "bg-gray-200 text-gray-700"}`}
                  >
                ...
                <div className="flex justify-end gap-2 pt-3 border-t border-gray-100 dark:border-zinc-800">
                  <button
                    className="p-2 rounded-lg bg-gray-50 dark:bg-zinc-800"
                    onClick={() =>
                      toggleArticleStatus(
                        article.id_article,
                        article.est_publie,
                  ...
                  <button
                    className="p-2 text-blue-600 bg-blue-50 dark:bg-blue-900/10 rounded-lg"
                  ...
                  <button
                    className="p-2 text-red-600 bg-red-50 dark:bg-red-900/10 rounded-lg"
                    onClick={() =>
                      deleteArticle(article.id_article, article.titre)
                    }
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-zinc-800">
                {articles.map((article) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-sm">
                      <span
                        className={`inline-flex items-center px-2.5 py-1 text-xs font-semibold rounded-full ${article.est_publie ? "bg-emerald-100 text-emerald-700" : "bg-gray-200 text-gray-700"}`}
                      >
                        {article.est_publie ? "Publié" : "Brouillon"}
                      </span>
                    </td>
      ...
      {activeTab === "exercices" && (
        <div className="w-full">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <button
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-lg font-medium transition-colors"
              onClick={() => setShowAddExerciceForm(!showAddExerciceForm)}
            >
              {showAddExerciceForm ? (
                <X className="w-4 h-4" />
              ) : (
          ...
          {showAddExerciceForm && (
            ...
            >
              <div className="grid gap-4 mb-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2 border rounded-lg dark:bg-zinc-800 dark:border-zinc-700"
                    type="text"
                    value={newExercice.nom_exercice}
                    onChange={(e) =>
                      setNewExercice({
                        ...newExercice,
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      type="number"
                      value={newExercice.temps_inspiration}
                      onChange={(e) =>
                        setNewExercice({
                          ...newExercice,
                          temps_inspiration: Number(e.target.value),
                        })
                      }
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      type="number"
                      value={newExercice.temps_apnee}
                      onChange={(e) =>
                        setNewExercice({
                          ...newExercice,
                          temps_apnee: Number(e.target.value),
                        })
                      }
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      type="number"
                      value={newExercice.temps_expiration}
                      onChange={(e) =>
                        setNewExercice({
                          ...newExercice,
                          temps_expiration: Number(e.target.value),
                        })
                      }
                    />
                  </div>
              ...
              <button
                className="w-full sm:w-auto px-6 py-2 bg-sky-600 hover:bg-sky-700 text-white font-medium rounded-lg"
                type="submit"
              >
                Créer l'exercice
              </button>
          ...
          <div className="md:hidden flex flex-col gap-4">
            {exercices.map((exo) => (
              ...
              >
                <span className="font-bold text-gray-900 dark:text-white text-base">
                  #{exo.id_exercice} - {exo.nom_exercice}
                </span>
                ...
                <div className="flex justify-end gap-2 pt-3 border-t border-gray-100 dark:border-zinc-800">
                  <button
                    className="p-2 text-blue-600 bg-blue-50 dark:bg-blue-900/10 rounded-lg"
                    onClick={() => setEditingExercice(exo)}
                  >
                    <Pencil className="w-4 h-4" />
                  </button>
                  <button
                    className="p-2 text-red-600 bg-red-50 dark:bg-red-900/10 rounded-lg"
                    onClick={() =>
                      deleteExercice(exo.id_exercice, exo.nom_exercice)
                    }
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
            ...
            >
              <div className="grid gap-4 sm:gap-5 mb-6">
                <div>
                  ...
                  <input
                    required
                    className="w-full p-2.5 border rounded-lg dark:bg-zinc-800 dark:border-zinc-700"
                    type="text"
                ...
                <div>
                  ...
                  <textarea
                    required
                    className="w-full p-2.5 border rounded-lg dark:bg-zinc-800 dark:border-zinc-700 resize-y"
                    rows={6}
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-gray-100 dark:border-zinc-800">
                <button
                  className="w-full sm:w-auto px-6 py-2.5 font-medium bg-gray-100 rounded-lg"
                  type="button"
                  onClick={() => setEditingArticle(null)}
                >
                  Annuler
                ...
                <button
                  className="w-full sm:w-auto px-6 py-2.5 font-medium text-white bg-blue-600 rounded-lg"
                  type="submit"
                >
                  Enregistrer
                </button>
          ...
          <div className="bg-white dark:bg-zinc-900 w-full max-w-lg rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[90dvh]">
            ...
            >
              <div className="grid gap-4 sm:gap-5 mb-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2.5 border rounded-lg dark:bg-zinc-800 dark:border-zinc-700"
                    type="text"
                    value={editingExercice.nom_exercice}
                    onChange={(e) =>
                      setEditingExercice({
                        ...editingExercice,
                        nom_exercice: e.target.value,
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      type="number"
                      value={editingExercice.temps_inspiration}
                      onChange={(e) =>
                        setEditingExercice({
                          ...editingExercice,
                          temps_inspiration: Number(e.target.value),
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      type="number"
                      value={editingExercice.temps_apnee}
                      onChange={(e) =>
                        setEditingExercice({
                          ...editingExercice,
                          temps_apnee: Number(e.target.value),
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      type="number"
                      value={editingExercice.temps_expiration}
                      onChange={(e) =>
                        setEditingExercice({
                          ...editingExercice,
                          temps_expiration: Number(e.target.value),
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-gray-100 dark:border-zinc-800">
                ...
                <button
                  className="w-full sm:w-auto px-6 py-2.5 font-medium text-white bg-sky-600 rounded-lg"
                  type="submit"
                >
                  Enregistrer
                </button>
